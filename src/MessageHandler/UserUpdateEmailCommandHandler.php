...
final class UserUpdateEmailCommandHandler implements CommandHandlerInterface
{
    ...
    public function __invoke(UserUpdateEmailCommand $message)
    {
        ...
        $this->eventBus->dispatch(new UserUpdatedEmailEvent($message->getPayload(), $message->getMetadata()));
        $this->dispatcher->dispatch(new GenericEvent($event), 'user.updated_email');
